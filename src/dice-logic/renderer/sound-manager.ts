...
export class SoundManager {
    ...
    onCollide(event: {
        body: { mass: number; velocity: { length(): number }; world: { stepnumber: number }; diceShape?: string };
        ...
        const now = Date.now();
        const stepNumber = (event.body as unknown as { world: { stepnumber: number } }).world.stepnumber;

        const currentSoundType = event.body.mass > 0 ? 'dice' : 'table';

        if ((this.lastSoundStep === stepNumber || this.lastSoundTime > now) && currentSoundType !== 'dice') return;
        if (
        ...
        if (event.body.mass > 0) {
            ...
            if (diceShape === 'd2' && this.dieSounds['coin']?.length) {
                sound = this.dieSounds['coin'][Math.floor(Math.random() * this.dieSounds['coin'].length)];
            } else if (this.dieSounds[DIE_MATERIAL]?.length) {
                sound = this.dieSounds[DIE_MATERIAL][Math.floor(Math.random() * this.dieSounds[DIE_MATERIAL].length)];
            }
