...
function overlapsAny(body: Body, others: readonly Body[]): boolean {
    return others.some((other) => body.position.distanceTo(other.position) < reach(body) + reach(other));
}
 ...
 * other nor with dice already in the air (F-022). Interpenetrating spawns make the solver
