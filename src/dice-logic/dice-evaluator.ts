    ...
    for (const roll of rolls) {
        if (!roll.exploded && matchesExplosionCondition(roll.value, sides, explode, customFaces)) {
            if (explode.compounding) {
                ...
                result.push({ ...roll, exploded: true, penetrating: explode.penetrating || undefined });
                const newVal = rollSingleDie(sides, customFaces, randomFn);
                counter.count++;
...
export function detectRerolls(node: DiceGroupNode, rawValues: number[], rolls?: DiceRoll[]): number[] {
...
export function detectExplosion(node: DiceGroupNode, rolls: DiceRoll[]): number[] {
    ...
    for (let i = 0; i < rolls.length; i++) {
        if (!rolls[i].exploded && matchesExplosionCondition(rolls[i].value, node.sides, explode, node.customFaces)) {
            indices.push(i);
        }
    }
    return indices;
...
function evaluateDiceGroup(
    ...
    if (preRolls) {
        ...
        if (node.fudge) {
            ...
            for (let i = 0; i < node.count; i++) {
                rolls.push({
                    sides: node.customFaces && node.customFaces.length > 0 ? Math.max(...node.customFaces) : node.sides,
    ...
    if (!preRolls) {
        if (node.modifiers.explode) {
            rolls = applyExplode(rolls, node.sides, node.modifiers.explode, node.customFaces, randomFn);
        }
        if (node.modifiers.reroll) {
            rolls = applyReroll(rolls, node.sides, node.modifiers.reroll, node.customFaces, randomFn);
        }
        if (node.modifiers.unique) {
            rolls = applyUnique(rolls, node.sides, node.modifiers.unique, node.customFaces, randomFn);
        }
    }
...
function evaluateAST(
    ...
    if (node.type === 'BinaryOp') {
        const leftResult = evaluateAST(node.left, operation, preGeneratedValues, groupIndex, randomFn);
        const rightResult = evaluateAST(node.right, node.operator, preGeneratedValues, groupIndex, randomFn);
        ...
        switch (node.operator) {
            ...
            case '*':
                ...
                break;
            case '/':
                value = rightResult.value !== 0 ? Math.floor(leftResult.value / rightResult.value) : 0;
                break;
            case '%':
                value = leftResult.value % rightResult.value;
                break;
    ...
    if (node.type === 'UnaryOp') {
        const operandResult = evaluateAST(node.operand, operation, preGeneratedValues, groupIndex, randomFn);
...
export function extractRawValuesFromAST(ast: ASTNode, randomFn?: () => number): Map<string, DiceRoll[]> {
