...
const run = (todo: string, tofix: string = TOFIX_OK) => validateBacklog({ todo, tofix, roadmap: ROADMAP });
...
describe('validateBacklog', () => {
    ...
    it('detects a duplicated identifier (E-ID-DUP)', () => {
        const todo = TODO_OK.replace('- [ ] ⬜ **T-002 — Planned thing**', '- [ ] ⬜ **T-001 — Duplicate id**');
        const violations = run(todo);
        expect(violations.some((v) => v.code === 'E-ID-DUP' && v.message.includes('T-001'))).toBe(true);
    });
    ...
    it('detects the legacy [/] marker (E-STATUS-AGREE)', () => {
        ...
        const violations = run(todo);
        expect(violations.some((v) => v.code === 'E-STATUS-AGREE' && v.message.includes('T-002'))).toBe(true);
    });
    ...
    it('detects a missing required field in a defect (E-FIELD-MISSING)', () => {
        const tofix = TOFIX_OK.replace('**Evidence:** Observed in the settings screen during manual testing.', '');
        const violations = run(TODO_OK, tofix);
        expect(violations.some((v) => v.code === 'E-FIELD-MISSING' && v.message.includes('Evidence'))).toBe(true);
    });
