    ...
    for (let i = 0; i < lines.length; i += 1) {
        ...
        if (line.startsWith('### ')) {
            ...
            if (!match) {
                violations.push({
                    ...
                    line: lineNumber,
                    code: 'E-ID-FMT',
                    message: 'entry heading does not match the canonical grammar "### F-### — Name"',
                });
                continue;
            }
            if (section === null) {
        ...
        if (/^\*\*(Version|Last updated):\*\*/i.test(line)) {
            violations.push({
                ...
                line: lineNumber,
                code: 'E-SECT-UNKNOWN',
                message: 'self-version/recency headers are illegal; git owns versioning and recency',
            });
        }
    }
    closeEntry();
...
const main = (): number => {
    ...
    const paths = {
        todo: resolve(todoArg ?? 'specs/TODO.md'),
        tofix: resolve(tofixArg ?? 'specs/TOFIX.md'),
        roadmap: resolve(roadmapArg ?? 'specs/ROADMAP.md'),
    };
